...
{	
	public function __construct() {
        parent::__construct();
        $this->load->model('income_data');
        $this->load->model('M_report_laba_rugi');
        
    }
    
    public function get_data_income(){
        $yearnow = date('2017.11');
        //$pastyear = mktime();
        $yearpast = date('2016.11');
        $comp = 7000;
        //echo $time;
        $dt1 = $this->income_data->get_data($yearnow, $comp);
        //$dt2 = $this->income_data->get_data($yearpast, $comp);
        //print_r($dt1);
        echo json_encode(array('data'=>$dt1));
    }
