<link href="<?php echo base_url(); ?>assets/marketshare/jquery_ui/jquery-ui.min.css" rel="stylesheet">
<script src="<?php echo base_url(); ?>assets/marketshare/jquery_ui/jquery-ui.min.js"></script>
<style>
    #loading_purple {
        position:fixed;
        top:0;
        left:0;
        background:url('<?php echo base_url(); ?>assets/marketshare/img/loading.gif')no-repeat center center;
        z-index:9999;
        text-align:center;
        width:100%;
        height:100%;
	...
	#chartobject-1{margin-top:50px !important;}
    .radio-inline{
        color:#000;
    }
    .modal-dialog{
    ...
    .fusioncharts-legend{width:100% !important;position:absolute;}
    .fusioncharts-legenditem{}
</style>
        ...
        <div class="panel-title">
            ...
            <div class="col-md-3" style="border-right: #005fbf;">
                ...
                <select id="org" class="form-control">
                    <option value="1" <?php echo (!isset($opco) || empty($opco))? "selected/" : ""; ?> >SMI Group</option>
                    <option value="7000" <?php echo (isset($opco) && $opco==7000)? "selected/" : ""; ?> >Semen Gresik</option>
                    <option value="3000" <?php echo (isset($opco) && $opco==3000)? "selected/" : ""; ?> >Semen Padang</option>
                    <option value="4000" <?php echo (isset($opco) && $opco==4000)? "selected/" : ""; ?> >Semen Tonasa</option>
                </select>
            </div>      
            <div class="col-md-1" style="display:none;">
                <?php if ($this->session->userdata('akses') == 1) { ?>
                <a href="<?=base_url('Upload')?>" target="_blank"><button class="btn btn-info" style="margin-top:24px"><i class="fa fa-upload"></i> Upload</button></a>
                <?php } ?>
            </div>
            <div class="col-md-3">
                <label>Bulan</label>
            ...
            <div class="col-md-1">
                <button id="filter" class="btn btn-success" style="margin-top:28px"><i class="fa fa-search"></i></button>
            </div>
        </div>        
    </div>
...
<script>
	var org = <?php echo (isset($opco) && !empty($opco))? $opco : 1; ?>;
	...
	function peta() {
		var peta;
		var url = base_url + 'MarketShare/getData/' + org + '/' + tahun + '/' + bulan;
		$.ajax({
			url: url,
			type: 'post',
			dataType: 'json',
	...
	function getDetail(prov) {
		var url = base_url + 'MarketShare/getDetail/' + org + '/' + prov + '/' + tahun + '/' + bulan;
		$.ajax({
			url: url,
			type: 'post',
			dataType: 'json',
	...
	function updateTime() {
		var url = base_url + 'MarketShare/getUpdateDate';
		$.ajax({
			url: url,
			type: 'post',
			dataType: 'json',
	...
	function summary() {
		var url = base_url + 'MarketShare/getSummary/' + tahun + '/' + bulan;
		$.ajax({
			url: url,
			type: 'post',
			dataType: 'json',
	...
	function getGrowthASI(kd_perusahaan) {
		...
		var url = base_url + 'MarketShare/getChartGrowth/' + kd_perusahaan;
		$.ajax({
			url: url,
			type: 'post',
			data: {
	...
	function getMarketVolume(kd_perusahaan) {
		...
		var _tglAwal = _period.find('select[name=tahunAwal]').val() + '' + _period.find('select[name=bulanAwal]').val();
		var _tglAkhir = _period.find('select[name=tahunAkhir]').val() + '' + _period.find('select[name=bulanAkhir]').val();
		var url = base_url + 'MarketShare/marketVolume/' + kd_perusahaan;
		$.ajax({
			url: url,
			type: 'post',
			data: {
	...
	function saveXls() {
		...
		var _tglAwal = _period.find('select[name=tahunAwal]').val() + '' + _period.find('select[name=bulanAwal]').val();
		var _tglAkhir = _period.find('select[name=tahunAkhir]').val() + '' + _period.find('select[name=bulanAkhir]').val();
		var url = base_url + 'MarketShare/dataHistory/' + _tglAwal + '/' + _tglAkhir;
		window.open(url);
	}
	...
	$(function () {
		...
		$("input[name='optradio']").click(function () {
			...
			var _tglAwal = _period.find('select[name=tahunAwal]').val() + '' + _period.find('select[name=bulanAwal]').val();
			var _tglAkhir = _period.find('select[name=tahunAkhir]').val() + '' + _period.find('select[name=bulanAkhir]').val();
			var url = base_url + 'MarketShare/getChartGrowth/1';
			$.ajax({
				url: url,
				type: 'post',
				data: {
		...
		$("input[name='optradio2']").click(function () {
			...
			var _tglAwal = _period.find('select[name=tahunAwal]').val() + '' + _period.find('select[name=bulanAwal]').val();
			var _tglAkhir = _period.find('select[name=tahunAkhir]').val() + '' + _period.find('select[name=bulanAkhir]').val();
			var url = base_url + 'MarketShare/marketVolume/1';
			$.ajax({
				url: url,
				type: 'post',
				data: {
