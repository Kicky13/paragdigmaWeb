...
$(function () {
		$('#bulan_<?php if(!empty($input['periode'])) { echo $input['periode'];}else{echo date('n');} ?>').addClass('active btn-danger');
		$('#Tahun_<?php if(!empty($input['tahun'])) {echo $input['tahun'];}else{echo date('Y');} ?>').addClass('active btn-danger');
    $('#container').highcharts({
        ...
        xAxis: [{
            categories: [<?php $SumDate = date('t', mktime(0, 0, 0, date('m'), 1, date('Y'))); 
								echo '1';
								for( $i=2;$i <= $SumDate; $i++ ){
									echo ", $i";
								} 
...
opposite: true,tickPositions: [<?php echo $Interval[0]; ?>]
		...
		//	zIndex: 1,
			...
			borderRadius: 5,
            data: [
			<?php if(!empty($DataTerak[1]['ton_real'])){
...
//	Atas Chart 1
			...
			color:'#000099',
            data: [
					<?php if(!empty($DataTerak[1]['avg_real'])){ 
			...
			color:'#FF007F',
            data: [
				<?php if(!empty($DataTerak[1]['accum_budget'])){ echo $DataTerak[1]['accum_budget'];
						for($i=2;$i <= count($DataTerak) ; $i++){
			...
			zIndex: 4,
            data: [
			<?php if(!empty($DataTerak[1]['accum_prognosa'])){
			...
			color:'#FFFF00',
            data: [
			<?php if(!empty($DataTerak[1]['accum_real'])){
            ...
            data: [
			<?php if(!empty($DataTerak[1]['ton_design'])){
		 ...
		 function (chart) {
            var max = <?php if(!empty($DataTerak[1]['ton_design'])){ echo $DataTerak[1]['ton_design'];}else{echo 0;}?>;
		...
		function Btn_periode(periode){
			...
			var form = $('<form action="' + url + '" method="get">' +
			  ...
			  '<input type="hidden" name="input[tahun]" value="<?php if(!empty($input['tahun'])) {echo $input['tahun'];}else{ echo date('Y');} ?>" />' +
			   '<input type="hidden" name="plant" value="<?php echo $this->input->get('plant'); ?>" />' +
		...
		var url= "";
			var form = $('<form action="' + url + '" method="get">' +
			  '<input type="hidden" name="input[periode]" value="<?php if(!empty($input['periode'])) { echo $input['periode'];}else{echo date('n');} ?>" />' +
...
<div id="LoadDetail"></div>
			...
			<div class="col-lg-3" style="padding:0px;">
				<div class="panel panel-default" style="padding:5px;border:none;border-radius: 0 !important;">
					<strong>Design Capacity</strong> <?php if(!empty($DataTerak[1]['ton_design'])){ echo number_format($DataTerak[1]['ton_design']);}else{echo 0;}?> Ton/ Day <br>
