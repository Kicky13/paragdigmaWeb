...
?>]
                    ...
                    data: [
<?php
if (!empty($ton_real[0]['CEMENT'])) {
    echo $ton_real[0]['CEMENT'];
    for ($i = 1; $i < count($ton_real); $i++) {
        echo "," . $ton_real[$i]['CEMENT'];
    }
} else {
    echo 0;
}
?>
                ...
                },
                {
                    ...
                    data: [<?php
for ($x = 0; $x < $last_update - 1; $x++) {
    $acc_real += $ton_real[$x]["CEMENT"];
    echo $acc_real . ",";
}
?>]
                }, {
...
for ($x = 1; $x < $SumDate; $x++) {
    if ($x <= $last_update) {
        $prog += $ton_real[$x]["CEMENT"];
    } else {
        $prog += $prognose[$x]['FM_PROGNOSE'];
    }
    echo $prog . ",";
}
?>]
...
?>
    ...
    function Btn_periode(periode) {
        var url = "<?= base_url() ?>index.php/eksekutif_report/Cement_1";
//        var url = "";
...
<div style="padding-left: 50px">
    <div class="panel panel-default" style="padding:5px;border:none;border-radius: 0 !important;">
        <div style="padding:0px;">
            ...
            <?php
            if (!empty($des_cm['0']["CEMENT"])) {
                echo '<b style="font-style: italic;">' . number_format($des_cm['0']["CEMENT"]) . '</b>';
            } else {
