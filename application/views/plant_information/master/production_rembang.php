    ...
    <nav class="navbar navbar-inverse my_margin">
        <div class="col-md-6 container-fluid">
            <div class="navbar-header">
                <a class="navbar-brand" href="javascript:void(0)">Gresik Cement Production Report</a>
            </div>
        </div>
        <div class="col-md-6">
...
<script>
    ...
    hot = new Handsontable($container[0], {
        ...
        data: [
            ['Date', 'R.MILL #1', '', 'KILN #1', '', 'F.MILL #1', '', 'F.MILL #2', ''],
    ...
    $("#load").click(function () {
        $.ajax({
            url: 'master_data/get_production_rembang/<?php echo $bulan; ?>/<?php echo $tahun; ?>',
            type: 'GET',
            success: function (res) {
                var mine = JSON.parse(res);
                var data = [], row;
                data[0] = ['Date', 'R.MILL #1', '', 'KILN #1', '', 'F.MILL #1', '', 'F.MILL #2', ''];
                ...
                for (var i = 0, ilen = mine.mydata.length; i < ilen; i++) {
                    ...
                    row[6] = mine.mydata[i].FM1_JOP;
                    row[7] = mine.mydata[i].FM2_PROD;
                    row[8] = mine.mydata[i].FM2_JOP;
    ...
    function Btn_periode(periode) {
        var url = "<?= base_url() ?>index.php/master_data/rembang";
