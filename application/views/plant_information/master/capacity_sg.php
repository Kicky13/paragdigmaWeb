...
if ($plant == 1) {
    $bantu = 'Finish Mill / Tuban 1';
} elseif ($plant == 2) {
    $bantu = 'Finish Mill / Tuban 2';
} elseif ($plant == 3) {
    $bantu = 'Finish Mill / Tuban 3';
} elseif ($plant == 4) {
    $bantu = 'Finish Mill / Tuban 4';
} elseif ($plant == 5) {
    $bantu = 'Finish Mill 5';
} elseif ($plant == 6) {
    $bantu = 'Finish Mill 6';
            ...
            <form name="load" id="load" style="width: 100%;">
                <div class="col-md-4" style="padding-left: 0px">
                    <ul class="nav navbar-nav navbar-center">
                        <li class="dropdown">
                            <?php
                            $jenis_arr = array(1 => "Finish Mill / Tuban 1", 2 => "Finish Mill / Tuban 2", 3 => "Finish Mill / Tuban 3",
                                4 => "Finish Mill / Tuban 4", 5 => "Finish Mill 5", 6 => "Finish Mill 6", 7 => "Finish Mill 7",
                                8 => "Finish Mill 8", 9 => "Finish Mill 9", 10 => "Finish Mill A", 11 => "Finish Mill B", 12 => "Finish Mill C",
                                13 => "Finish Mill D", 14 => "Rembang 1", 15 => "Rembang 2", 16 => "Cigading");
                            ?>
...
<div class="col-md-12">
    ...
    <div class="col-md-6">
        ...
        <div class="col-md-12" style="margin-top: 10px;">
            ...
            <p style="margin-top: 10px;">Don't have any template before? <a href="<?php base_url()?>upload/template/TEMPLATE_UPLOAD_CAPACITY.xls" download>Download this template</a></p>
        </div>
    </div>
</div>
<script>
    ...
    hot = new Handsontable($container[0], {
        ...
        cells:  function (row, col, prop) {
            ...
            if (row <= 1 && col > 0) {
                if (col < 1)
                    cellProperties.renderer = Pojok; // uses function directly
                else if (col < 3)
                    cellProperties.renderer = Capacity;
                else if (col >= 2)
                    cellProperties.renderer = Budget;
            }
            if (row <= 1 && col < 1){
        ...
        afterChange: function (change, source) {
            ...
            $.ajax({
                url: '<?=site_url()?>/master_data/save_capacity_sg',
                dataType: 'json',
                type: 'POST',
    ...
    function Budget(instance, td, row, col, prop, value, cellProperties) {
        Handsontable.renderers.TextRenderer.apply(this, arguments);
    ...
    $("#load").click(function () {
        $.ajax({
            url: 'master_data/get_capacity_sg/<?php echo $pemisah; ?>/<?php echo $plant; ?>/<?php echo $tahun; ?>',
            type: 'GET',
            success: function (res) {
                var mine = JSON.parse(res);
                var data = [], row;
                data[0] = ['BULAN', 'CAPACITY', '', 'BUDGET', ''];
                data[1] = ['', 'CEMENT', 'CLINKER', 'CEMENT', 'CLINKER'];
                ...
                for (var i = 0, ilen = mine.mydata.length; i < ilen; i++) {
                    ...
                    row[3] = mine.mydata[i].RKAP_CEMENT;
                    row[4] = mine.mydata[i].RKAP_CLINKER;
