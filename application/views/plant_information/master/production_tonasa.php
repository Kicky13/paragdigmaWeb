    ...
    <nav class="navbar navbar-inverse my_margin">
        <div class="col-md-6 container-fluid">
            <div class="navbar-header">
                <a class="navbar-brand" href="<?= base_url() ?>index.php/master_data/tlcc">Tonasa Cement Production Report</a>
            </div>
        </div>
        <div class="col-md-6">
...
<script>
    ...
    hot = new Handsontable($container[0], {
        ...
        data: [
            ['Date', 'R.MILL #2', '', 'R.MILL #3', '', 'R.MILL #4.1', '', 'R.MILL #4.2', '', 'R.MILL #5.1', '', 'KILN #2', '', 'KILN #3', '', 'KILN #4', '', 'KILN #5', '', 'FM #2', '', 'FM #3', '', 'FM #4.1', '', 'FM #4.2', '', 'FM #5.1', '', 'FM #5.2', ''],
            ['', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP']
        ],
        mergeCells: [
    ...
    $("#load").click(function () {
        $.ajax({
            url: 'master_data/get_production_tonasa/<?php echo $bulan; ?>/<?php echo $tahun; ?>',
            type: 'GET',
            success: function (res) {
                var mine = JSON.parse(res);
                var data = [], row;
                data[0] = ['Date', 'R.MILL #2', '', 'R.MILL #3', '', 'R.MILL #4.1', '', 'R.MILL #4.2', '', 'R.MILL #5.1', '', 'KILN #2', '', 'KILN #3', '', 'KILN #4', '', 'KILN #5', '', 'FM #2', '', 'FM #3', '', 'FM #4.1', '', 'FM #4.2', '', 'FM #5.1', '', 'FM #5.2', ''];
                data[1] = ['', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP', 'PROD', 'JOP'];
                for (var i = 0, ilen = mine.mydata.length; i < ilen; i++) {
                    ...
                    row[9] = mine.mydata[i].RM5_PROD;
                    row[10] = mine.mydata[i].RM5_JOP;
                    ...
                    row[17] = mine.mydata[i].KL5_PROD;
                    row[18] = mine.mydata[i].KL5_JOP;

                    row[19] = mine.mydata[i].FM2_PROD;
                    row[20] = mine.mydata[i].FM2_JOP;
                    row[21] = mine.mydata[i].FM3_PROD;
                    row[22] = mine.mydata[i].FM3_JOP;
                    row[23] = mine.mydata[i].FM41_PROD;
                    row[24] = mine.mydata[i].FM41_JOP;
                    row[25] = mine.mydata[i].FM42_PROD;
                    row[26] = mine.mydata[i].FM42_JOP;
                    row[27] = mine.mydata[i].FM51_PROD;
                    row[28] = mine.mydata[i].FM51_JOP;
                    row[29] = mine.mydata[i].FM52_PROD;
                    row[30] = mine.mydata[i].FM52_JOP;
    ...
    function klikTbl() {
        ...
        if (bulan === null || bulan === 0 || bulan === 'undefined') {
            ...
            location.href = 'master_data/xls_ST/<?php echo $bulan; ?>/<?php echo $tahun; ?>';
        }
    }
    
    function Btn_periode(periode) {
        var url = "<?= base_url() ?>index.php/master_data/tonasa";
