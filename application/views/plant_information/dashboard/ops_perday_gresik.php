    ...
    <nav class="navbar navbar-inverse my_margin">
        <div class="col-md-6 container-fluid">
            <div class="navbar-header">
                <a class="navbar-brand" href="javascript:void(0)">Gresik Yield Report</a>
            </div>
        </div>
        <div class="col-md-6"> <!--   container-fluid -->
                    ...
                    <ul class="nav navbar-nav navbar-right">
                        <li class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li class="active"><a href="<?= base_url() ?>index.php/plant_dashboard/gresik">Gresik</a></li>
                                <li role="separator" class="divider"></li>

                                <li><a href="<?= base_url() ?>index.php/plant_dashboard/tonasa">Tonasa</a></li>
                                <li role="separator" class="divider"></li>
...
<div class="col-md-12" style="padding: 1px; margin-bottom: 5px">
    ...
    <div class="col-md-8" style="font-size: 16px; padding-top: 5px" id="exampleConsole"><strong class="blink_me">Select month and year then click Load</strong></div>
    <div class="col-md-2">
...
<script>
    ...
    hot = new Handsontable($container[0], {
        ...
        data: [
            ['Date', 'Yield in %', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''],
            ['', 'KL1', 'KL2', 'KL3', 'KL4', 'FM1', 'FM2', 'FM3', 'FM4', 'FM5', 'FM6', 'FM7', 'FM8', 'FM9', 'FMA', 'FMB', 'FMC']
        ],
        mergeCells: [
    ...
    $("#load").click(function () {        
        $.ajax({
            url: 'plant_dashboard/ops_day_SG/<?php echo $tahun; ?>/<?php echo $bulan; ?>',
            type: 'GET',
            success: function (res) {
                var mine = JSON.parse(res);
                var data = [], row;
                data[0] = ['Date', 'Yield in %', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''];
                data[1] = ['', 'KL1', 'KL2', 'KL3', 'KL4', 'FM1', 'FM2', 'FM3', 'FM4', 'FM5', 'FM6', 'FM7', 'FM8', 'FM9', 'FMA', 'FMB', 'FMC'];
                for (var i = 0, ilen = mine.mydata.length; i < ilen; i++) {
                    row = [];
                    row[0] = mine.mydata[i].DATE_PROD;
                    row[1] = mine.mydata[i].KL1_PROD;
                    row[2] = mine.mydata[i].KL2_PROD;
                    row[3] = mine.mydata[i].KL3_PROD;
                    row[4] = mine.mydata[i].KL4_PROD;

                    row[5] = mine.mydata[i].FM1_PROD;
                    row[6] = mine.mydata[i].FM2_PROD;
                    row[7] = mine.mydata[i].FM3_PROD;
                    row[8] = mine.mydata[i].FM4_PROD;
                    row[9] = mine.mydata[i].FM5_PROD;
                    row[10] = mine.mydata[i].FM6_PROD;
                    row[11] = mine.mydata[i].FM7_PROD;
                    row[12] = mine.mydata[i].FM8_PROD;
                    row[13] = mine.mydata[i].FM9_PROD;
                    row[14] = mine.mydata[i].FMA_PROD;
                    row[15] = mine.mydata[i].FMB_PROD;
                    row[16] = mine.mydata[i].FMC_PROD;
    ...
    function Btn_periode(periode) {
        var url = "<?= base_url() ?>index.php/plant_dashboard/gresik";
