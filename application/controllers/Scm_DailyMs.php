...
class Scm_DailyMs extends CI_Controller {

    function __construct() {
        parent::__construct();
        date_default_timezone_set("Asia/Jakarta");
        $this->load->model('scm_dailyms_model','DailyMs_model');
//        error_reporting(0);
    }
    ...
    function getData($org, $time=null) {
        if ($time==null)
        {
            $time = date("Y.m");
        }
...
//            $data = $this->DailyMs_model->datasmig($tahun, $bulan, $hari);
        ...
        if ($org == 1) {// peta data SMIG
            foreach ($prov as $key => $row) {
                ...
                $arrData[] = array("NAMA_PROV"=>$row['NM_PROV'],
                                    "PERSEN_AKTUALMS"=>$realrkap,
                                    "PERSEN_TARGETMS"=>$targetrkap,
                                    "REALISASI"=>$realisasi,
                                    "DEMAND"=>$demand
                                    );
    ...
    // coba123
       function getData_new($org, $time=null) {
        if ($time==null)
        {
            $time = date("Y.m");
        }
