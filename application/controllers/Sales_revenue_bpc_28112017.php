...
class Sales_revenue_bpc extends CI_Controller {
     ...
     * @return [type]       [format return json]
     */
    function globalVolumeRevenue_SMIG_clinker($time=null)
    {
        if ($time==null)
        {
            $time = date("Y.m");
    ...
    function VolumeRevenue_PER_OPCO($opco,$time=null)
    {
        ...
        if ($opco=="7000")
        {
            ...
            $data = $this->get_data_company_6000($opco, $b, $thn);
        }else{
            $listopco = $opco;
            $data = $this->get_data_company($opco, $b, $thn);
        }
    ...
    function VolumeRevenue_BBC_PER_OPCO_clinker($opco,$time=null)
    {
        if ($time==null)
        {
            $time = date("Y.m");
    ...
    function VolumeRevenue_PROV_PER_OPCO_by_material($opco,$material,$time=null)
    {
        if ($time==null)
        {
            $time = date("Y.m");
        ...
        $i = 0;
        //$data = $this->get_prov_all_opco($opco, $b, $thn);
        $data = $this->get_prov_all_opco_and_material($opco, $b, $thn,$material);
        echo json_encode($data);
        //echo json_encode($data);
        // if (substr($opco, 0, 2) == '10' or substr($opco, 0, 2) == '60') {
        //     $this->companyprov($opco, $b, $thn);
    ...
    public function get_prov_all_opco_and_material($opco,$bln, $thn, $material) {
        $data = $this->M_RVD->get_prov_by_opco($opco,$bln,$thn);
        // echo "[";
        $count = count($data);
        $u = 1;
        ...
        foreach ($data as $d) {
            //$opcoperprov = $this->get_data_prov($d["VKORG"], $d["VKBUR_TXT"], $bln, $thn, $d["VKBUR"]);
            $opcoperprov = $this->get_data_prov_and_material($d["VKORG"], $d["VKBUR_TXT"], $bln, $thn, $d["VKBUR"],$material);
            $datareturn[] = array("COMPANY"=>$opco,"IDPROV"=>$d["VKBUR"],"PROV"=>$d["VKBUR_TXT"],"data"=>$opcoperprov);
            // if ($u == $count) {
            //     echo '{"idd":"3000__4000__5000__7000pisah' . $d["VKBUR_TXT"] . 'pisah' . $d["VKBUR"] . '","company":" <b>' . $d["VKBUR_TXT"] . '</b>",' . $this->get_data_prov($d["VKORG"], $d["VKBUR_TXT"], $bln, $thn, $d["VKBUR"]) . '"state":"closed"}';
            // } else {
    ...
    public function data1_clinker($bln, $thn) {
    //     $data = '{"total":50,"rows":[
                //  {"idd":"smig","company":"<b>Semen Indonesia</b>",' . $this->total($bln, $thn) . '"state":"closed"}
                // ],"footer":[
                //  {"company":"TOTAL",' . $this->total($bln, $thn) . '"state":""}
                // ]}';
        $data = array("company"=>"Semen Indonesia","data"=>$this->total_clinker($bln, $thn));
        return $data;
    }
    ...
    public function get_data_prov_and_material($comp, $prov, $bln, $thn, $no_prov,$material) {
        $tgl_now = date('Y/m/d');
        $bln_now = date('m');
        $thn_now = date('Y');
        $tgl_last = date("t", strtotime($thn . '/' . $bln . '/23'));
        ...
        $data = $this->M_RVD->data_prov_and_material($comp, $prov, $tgl_now, $tgl_rel, $bln, $thn, $tgl_last);
        //echopre($data);exit;
        $vol = array();
        $price = array();
        $totsales = array();
        ...
        //$data_rkap = $this->M_RVD->get_rkap_bulanan_prov($comp, $no_prov, $thn, $bln);
        $data_rkap = $this->M_RVD->get_rkap_bulanan_prov_and_material($comp, $no_prov, $thn, $bln, $material);
        $rkap_ton = array();
        $rkap_rev = array();
        $material = array();
    ...
    public function get_data_company_6000($comp, $bln, $thn) {
            $tgl_now = date('Y/m/d');
        $bln_now = date('m');
        $thn_now = date('Y');
        $tgl_last = date("t", strtotime($thn . '/' . $bln . '/23'));
        ...
        $data_rkap = $this->M_RVD->get_rkap_bulanan_company_6000($comp, $thn, ($bln-1));
    ...
    public function total($bln, $thn) {
        ...
        $tgl_aja=date('d');
        $data_ac_volume = $this->M_RVD->total_ac_volume($tgl_aja, $tgl_rel, ($bln-1), $thn, $tgl_last);
        $data_ac_volume_harian = $data_ac_volume[0]['TOTAL'];
        $data_ac_volume_bulanan = $data_ac_volume[1]['TOTAL'];
        $data_ac_volume_tahunan = $data_ac_volume[2]['TOTAL'];
        
        $data_ac_bulan = $this->M_RVD->total_bulan($tgl_aja, $tgl_rel, ($bln-1), $thn, $tgl_last);
        $data_ac_volume_bulanan = $data_ac_bulan[0]['VOL'];

        $tgl_terakhir = date('t');
        $data_rkap_volume = $this->M_RVD->RKAP_SMIG($thn,($bln-1),$tgl_terakhir);
        $data_rkap_volume_harian = $data_rkap_volume[0]['TOTAL'];
        $data_rkap_volume_bulanan = $data_rkap_volume[1]['TOTAL'];
        $data_rkap_volume_tahunan = $data_rkap_volume[2]['TOTAL'];
        ...
        $varianVolumeto_day =abs(floatval(str_replace(",", "", $data_rkap_volume_harian))-floatval(str_replace(",", "", $data_ac_volume_harian)))  ;
        $varianVolumeto_day = number_format($varianVolumeto_day,0);
        $varianVolumemonth_to_day = abs(floatval(str_replace(",", "", $data_rkap_volume_bulanan))-floatval(str_replace(",", "", $data_ac_volume_bulanan)));
        $varianVolumemonth_to_day = number_format($varianVolumemonth_to_day,0);
        $varianVolumeyear_to_day = abs(floatval(str_replace(",", "", $data_rkap_volume_tahunan))-floatval(str_replace(",", "", $data_ac_volume_tahunan)));
        $varianVolumeyear_to_day = number_format($varianVolumeyear_to_day,0);
        ...
        $data_r[] = array("to_day"=>array("DATAVOLUME"=>array(
            array("VOLUME"=>$data_ac_volume_harian,"TARGETVOLUME"=>$data_rkap_volume_harian,"VARIANVOLUME"=>$varianVolumeto_day)
            ),
            "DATAREVENUE"=>array(
            array("PRICE"=>$this->convertmoneytomio($price[0]),"TOTALSALES"=>$this->convertmoneytomio($totsales[0]),"OA"=>$this->convertmoneytomio($oa[0]),"REVENUE"=>$this->convertmoneytomio($rev[0]),"TARGETREVENUE"=>$this->convertmoneytomio($rkap_reve[0]),"VARIANREVENUE"=>$this->convertmoneytomio($varianRevenueto_day))
            )
        ...
        $data_r[] = array("month_to_day"=>array("DATAVOLUME"=>
            array("VOLUME"=>$data_ac_volume_bulanan,"TARGETVOLUME"=>$data_rkap_volume_bulanan,"VARIANVOLUME"=>$varianVolumemonth_to_day)
            ,
            "DATAREVENUE"=>array("PRICE"=>$this->convertmoneytomio($price[1]),"TOTALSALES"=>$this->convertmoneytomio($totsales[1]),"OA"=>$this->convertmoneytomio($oa[1]),"REVENUE"=>$this->convertmoneytomio($rev[1]),"TARGETREVENUE"=>$this->convertmoneytomio($rkap_rev[1]),"VARIANREVENUE"=>$this->convertmoneytomio($varianRevenuemonth_to_day))
            ));

        $data_r[] = array("year_to_day"=>
                            array("DATAVOLUME"=>array("VOLUME"=>$data_ac_volume_tahunan,"TARGETVOLUME"=>$data_rkap_volume_tahunan,"VARIANVOLUME"=>$varianVolumemonth_to_day
                                                     ),
                                 "DATAREVENUE"=>array("PRICE"=>$this->convertmoneytomio($price[2]),"TOTALSALES"=>$this->convertmoneytomio($totsales[2]),"OA"=>$this->convertmoneytomio($oa[2]),"REVENUE"=>$this->convertmoneytomio($rev[2]),"TARGETREVENUE"=>$this->convertmoneytomio($rkap_rev[2]),"VARIANREVENUE"=>$this->convertmoneytomio($varianRevenuemonth_to_day)
                                                    )
                                 )
    ...
    public function total_clinker($bln, $thn) {
        $tgl_now = date('Y/m/d');
        $bln_now = date('m');
        $thn_now = date('Y');
        $tgl_last = date("t", strtotime($thn . '/' . $bln . '/23'));
        ...
        $data = $this->M_RVD->total_clinker($tgl_now, $tgl_rel, $bln, $thn, $tgl_last);
        //echopre($data);exit;
        $vol = array();
        $price = array();
        $totsales = array();
