...
class Sales_revenue_imam extends CI_Controller
{	
...
//        
        ...
        $data_rkap = $this->M_RVD->get_detail_rkap_harian_smig($bln_now, $thn_now,$tgl_last);
        $akumulasi_rkap=array();
        $i=0;
        $rkap=0;
        foreach ($data_rkap as $key => $value) {
            if ($value["RKAP_VOLUME"]!="")
            {
                $rkap += $value["RKAP_VOLUME"];
                $akumulasi_rkap[$i]= array("tanggal"=> ($i+1)."-".date("M-Y"), "nilai"=> $rkap);
                $i++;
            }
        }
        // echo "<pre>";
        // print_r($akumulasi_rkap);
        // exit;
        #######################################################################################
    //    echo json_encode(array('nama'=>'data_prognus_harian','rkap_lama'=>$data_rkap,'rkap_baru'=>$data_rkap2));
        echo json_encode(array('nama'=>'data_prognus_harian_smig','prognus_rkap_volume'=>$data_prognus_volume_rkap,'prognus_rkap_revenue'=>$data_prognus_revenue_rkap,
            'rata_rkap_revenue'=>$data_rata_rkap_revenue,'rata_rkap_volume'=>$$akumulasi_rkap,'rata_capaian_volume'=>$data_rata_capaian_volume,
...
//        
    ...
    //    echo json_encode(array('nama'=>'data_prognus_harian','rkap_lama'=>$data_rkap,'rkap_baru'=>$data_rkap2));
    //    $data_prognus_volume_rkap
        echo json_encode(array('nama'=>'data_prognus_harian_smig','prognus_rkap_volume'=>$akumulasi_rkap,'prognus_rkap_revenue'=>$data_prognus_revenue_rkap,
